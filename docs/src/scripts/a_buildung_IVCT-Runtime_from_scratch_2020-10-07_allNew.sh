...
#
# 10.03.2020 brf, Fraunhofer IOSB
# Fraunhofer Institut für Optronik,Systemtechnik und Bildauswertung
#
#
# Info and Script to build a IVCT_Runtime environment with all necessary Parts
#
...
#
# versionen      brf
# 20201007  brf  little improvements and corrections
# 20200518  brf  build all new, with unknown Version-Numbers
...
export IVCTTOOL_Dir=/home1_phys/IVCTool
...
##  fuer weitergabe wieder aktivieren !
...
[ -d $RUNTIME_Dir/TestSuiteDefs ] && mv $RUNTIME_Dir/TestSuites $RUNTIME_Dir/TestSuiteDefs_${datum}_${zeit}
...
read -p "-- Break after got HelloWorld --, [Enter]  to continue, 'ctrl-c' to stop here"
...
read -p "-- Break after got LogSink --, [Enter]  to continue, 'ctrl-c' to stop here"
...
read -p "-- Break after got TCexec --, [Enter]  to continue, 'ctrl-c' to stop here"
...
read -p "-- Break after got UI --, [Enter]  to continue, 'ctrl-c' to stop here"
...
echo -e "-extract $TestSuiteDevelopment_Dir/TS_HelloWorld/build/distributions/$TSHelloWorldTar to $RUNTIME_Dir/TestSuites \n"

tar -xf $TestSuiteDevelopment_Dir/TS_HelloWorld/build/distributions/$TSHelloWorldTar -C $RUNTIME_Dir/TestSuites

read -p "-- Break after got TS_HelloWorld --, [Enter]  to continue, 'ctrl-c' to stop here"


echo -e "\n -------------------------------------------------------------------"
echo -e "## get different Testsuites from TS_HLA_BASE Repository "
...
# -----------------------------------------------------------------------------
# prepare apache-tomcat which we fetched in the 1. part  with our application 
# -----------------------------------------------------------------------------
...
#ivctGuiServerWar="ivct.gui.server##2.2.2-SNAPSHOT.war"
# or
ivctGuiServerWar=$(ls ${Framework_Dir}/GUI/nato.ivct.gui.server.app.war/build/libs | grep war | grep -v jar) 
...
#ivctGuiUiHtmlWar=ivct.gui.ui.html##2.2.2-SNAPSHOT.war
# or
ivctGuiUiHtmlWar=$(ls ${Framework_Dir}/GUI/nato.ivct.gui.ui.html.app.war/build/libs | grep war | grep -v jar) 
...
echo "" >> ${RUNTIME_Dir}/IVCT.properties
echo "RTI_ID=pRTI"  >> ${RUNTIME_Dir}/IVCT.properties
...
echo "ACTIVEMQ_USER=admin" >>  ${RUNTIME_Dir}/IVCT.properties
echo "ACTIVEMQ_PASSWORD=password" >>  ${RUNTIME_Dir}/IVCT.properties
echo "ACTIVEMQ_HOST=localhost" >>  ${RUNTIME_Dir}/IVCT.properties
echo "ACTIVEMQ_PORT=61616" >>  ${RUNTIME_Dir}/IVCT.properties
...
echo "IVCT_TS_DEF_HOME_ID=${RUNTIME_Dir}/TestSuiteDefs"
